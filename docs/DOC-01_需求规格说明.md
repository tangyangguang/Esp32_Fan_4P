...
本项目功能对齐成熟 ESP8266 项目 `/Users/tyg/dir/claude_dir/ESP12F_Fan_4P`，但硬件平台改为 ESP32，并使用 `/Users/tyg/dir/claude_dir/Esp32Base` 作为基础库。项目同时承担 Esp32Base 全模块实战验证职责。
...
| F-05 | 红外学习 | Web 触发学习，支持常见协议，记录加速、减速、停止、30min、1h、2h | 本项目 IRReceiverDriver/FanWeb |
...
| F-08 | REST API | 提供 `/api/status`、`/api/speed`、`/api/timer`、`/api/stop`、`/api/config`、`/api/ir/learn`、`/api/ir/status`、`/api/reset` | Esp32Base 路由 + 本项目 FanWeb |
...
| F-20 | NTP 时间 | 网络可用后日志和状态页可显示真实时间 | Esp32Base NTP |
...
| NF-04 | 安全 | Web Basic Auth，默认 `admin/admin123`，支持修改密码 |
...
| --- | --- | --- | --- |
| F-01, F-02, F-03, F-13 | FanDriver | Log | 待实现/验证 |
| F-04, F-18 | ButtonDriver, LedIndicator | Log | 待实现/验证 |
| F-05 | IRReceiverDriver, FanWeb | Web, Config, Log | 待实现/验证 |
| F-06, F-12, S-01 | FanController | Config, Log | 待实现/验证 |
| F-07, F-08, F-16 | FanWeb | Web, Config, Log | 待实现/验证 |
| F-09 | main, FanController | Fs, FileLog, Web Logs | 固件已编译通过，待实机验证 |
| F-10 | main | Ota, Web, Fs | 固件已编译通过，待实机验证 |
| F-11, F-15, F-17 | main | WiFi, DNS, mDNS, Web | 固件已编译通过，待实机验证 |
| F-14 | FanController | WiFi, Sleep | 待实现/实机验证 |
| F-19, F-20, S-02 | main | Watchdog, NTP, Health, System | 待实现/验证 |
