...
- 本项目是 `ESP12F_Fan_4P` 的 ESP32 版本，业务功能要对齐成熟 ESP8266 项目。
...
- Esp32Base Web 业务入口最终 API 为：

```cpp
Esp32BaseWeb::addPage(path, title, handler);
```

- 本项目业务页面应显式注册：

```cpp
Esp32BaseWeb::addPage("/fan", "Fan", FanWeb::handleStatusPage);
Esp32BaseWeb::addPage("/config", "Settings", FanWeb::handleConfigPage);
```
...
- `/esp32base/*` 是基础库管理页，包括 WiFi、OTA、Logs、Reboot 等。
